...
{
	ObjectWrap<AcDbLayerTableRecord> ltr_wrap(DBObject::OpenObjectById<AcDbLayerTableRecord>(id));
	...
	if (NULL != ent_wrap.object)
	{
		...
		ObjectWrap<AcDbLayerTableRecord> layer_wrap(DBObject::OpenObjectById<AcDbLayerTableRecord>(ent_wrap.object->layerId()));
...
AcGePoint3d Functions::GetBlkRefPos(const AcDbObjectId & br_id)
{
